...
export default function Dashboard() {
  ...
  const handleRenderTable = () => {
    if(tipoTabla == 'Alumnos') {
      return <CardLineChart title={nombreTabla} month={months} data={Alumnos.NuevosAlumnos} />
    }
    if(tipoTabla == 'Ganancias') {
      return <CardLineChart title={nombreTabla} month={months} data={Alumnos.Ganancias} />
    }
    if(tipoTabla == 'Ventas') {
      return <CardLineChart title={nombreTabla} month={months} data={Alumnos.CantidadVentas} />
    }
  }
        ...
        <div className="w-full h-auto flex flex-col items-center justify-center pt-5 gap-y-10">
          ...
          <div className="w-full flex flex-wrap justify-around items-between gap-8">
            ...
            <div onClick={handleSetGanancias}>
              <CardDashboardItem title="Ganancias del Mes" data={Alumnos?.Ganancias[0]} />
            </div>
            <div onClick={handleSetVendidos}>
              <CardDashboardItem title="Cursos Vendidos este mes" data={Alumnos?.CantidadVentas[0]} />
            </div>
            <div>
              <CardDashboardItem title="Alumnos Totales" data={Alumnos?.AlumnosTotales} clickable={false} />
            </div>
          </div>
